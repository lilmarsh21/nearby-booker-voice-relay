...
function explicitEstimateDeliveryChange(text) {
  const t = normalizedText(text);
  if (!t) return '';
  const changeWords = ['instead', 'change', 'switch', 'rather', 'actually', 'only'];
  const hasChangeIntent = changeWords.some((word) => t.includes(word))
    || t.includes('not by')
    || t.includes("don't send")
    || t.includes('do not send');
  if (!hasChangeIntent) return '';
  const wantsSms = asksForTextMessage(text);
  const wantsEmail = asksForEmailMessage(text);
  if (t.includes('both') || (wantsSms && wantsEmail)) return 'both';
...
async function executeNbbTool(state, name, callId, args = {}) {
  ...
  // caller number is already verified. A dictated email is confirmed once; after
  // confirmation the Relay freezes the canonical address for the rest of the call
  // so model retries cannot trigger duplicate confirmations or silently mutate it.
  if (['nbb_prepare_booking', 'nbb_send_estimate', 'nbb_request_callback'].includes(name) && toolArgs?.customer && typeof toolArgs.customer === 'object') {
    ...
    const effectiveEstimateDelivery = state.estimateDeliveryPreference || normalizeEstimateDelivery(toolArgs.delivery || '');
    ...
    if (candidatePhone && candidatePhone !== state.callerPhone && candidatePhone !== state.confirmedPhone) {
    ...
    if (usesEmail && candidateEmail && !state.confirmedEmail) {
  ...
  if (name === 'nbb_send_estimate') {
    ...
    // Delivery preference is durable call state, not a two-minute phrase timer.
    // Once selected, every retry uses the same normalized value until the caller
    // explicitly chooses another channel. This prevents loops like asking
    // "text/email/both?" again after the model already understood the answer.
    if (!state.estimateDeliveryPreference && requestedDelivery) {
      state.estimateDeliveryPreference = requestedDelivery;
      state.estimateDeliverySelectedAt = Date.now();
      pushInternalCallContext(state, `Estimate delivery preference is now locked to ${estimateDeliveryLabel(requestedDelivery)}. Do not ask for the delivery method again unless the caller explicitly changes it.`);
    }

    const delivery = state.estimateDeliveryPreference || requestedDelivery;
    if (!delivery) {
      acknowledgeFunction(state, callId, {
        success: false,
      ...
      requestModelResponse(
        state,
        'Ask one concise question for the estimate delivery preference: text, email, or both. After the caller answers, keep that choice for the rest of this estimate flow.',
    ...
    if ((wantsSms && state.rules?.sms_enabled === false) || (wantsEmail && state.rules?.email_enabled === false)) {
      ...
      requestModelResponse(state, 'Briefly explain that the selected delivery method is unavailable and offer only the other enabled delivery method if one is available.', 'estimate_delivery_unavailable', true);
      return;
    }
  ...
  try {
    ...
    if (name === 'nbb_send_estimate') {
      ...
      if (delivery) {
        ...
        state.estimateDeliverySelectedAt = state.estimateDeliverySelectedAt || Date.now();
      ...
      pushInternalCallContext(state, `The estimate send completed using ${estimateDeliveryLabel(delivery)}. Do not ask for delivery preference or reconfirm already-confirmed contact fields again.`);
    }
...
function handleOpenAIEvent(state, raw) {
  ...
  if (type === 'conversation.item.input_audio_transcription.completed') {
    ...
    if (transcript) {
      ...
      // Persist explicit delivery choices as normalized call state. This is an
      // optimization when ASR is clear; if ASR mangles a short answer, the
      // normalized nbb_send_estimate tool argument can establish the same state.
      let explicitDelivery = '';
      if (normalized === 'both' || (asksSms && asksEmail)) explicitDelivery = 'both';
      else if (asksSms) explicitDelivery = 'sms';
      else if (asksEmail) explicitDelivery = 'email';
      if (!state.estimateDeliveryPreference && explicitDelivery) {
        state.estimateDeliveryPreference = explicitDelivery;
        state.estimateDeliverySelectedAt = Date.now();
      } else if (state.estimateDeliveryPreference) {
        const changedDelivery = explicitEstimateDeliveryChange(transcript);
        if (changedDelivery && changedDelivery !== state.estimateDeliveryPreference) {
          state.estimateDeliveryPreference = changedDelivery;
          state.estimateDeliverySelectedAt = Date.now();
          pushInternalCallContext(state, `The caller explicitly changed estimate delivery to ${estimateDeliveryLabel(changedDelivery)}. Use this new delivery choice and do not ask again.`);
        }
      }
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.9' }));
    return;
  }
