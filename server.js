...
async function executeNbbTool(state, name, callId, args = {}) {
  ...
  if (['nbb_prepare_booking', 'nbb_send_estimate'].includes(name)) {
    toolArgs = mergeKnownCustomer(state, toolArgs);
  }

  // Precision contact fields: the Twilio caller number is already verified by the
  // call itself. An alternate phone or a newly dictated email must be confirmed
  // once before Voice is allowed to create/send a customer record with it.
  if (['nbb_prepare_booking', 'nbb_send_estimate'].includes(name) && toolArgs?.customer && typeof toolArgs.customer === 'object') {
    ...
    const candidateEmail = normalizedEmail(toolArgs.customer.email || '');
    const usesEmail = name === 'nbb_prepare_booking' || ['email', 'both'].includes(normalizedText(toolArgs.delivery || ''));
    ...
    if (usesEmail && candidateEmail && candidateEmail !== state.confirmedEmail) {
      ...
      requestModelResponse(state, `Before continuing, read back this email address exactly as captured: ${JSON.stringify(candidateEmail)}. Ask "Is that email correct?" Do not silently correct or guess any part of it, and do not call the booking/estimate tool again until the caller answers.`, 'email_confirmation_required', true);
      return;
    }
  }
  ...
  try {
    ...
    if (name === 'nbb_send_estimate') {
      ...
      if (delivery === 'sms' || delivery === 'text' || delivery === 'both') {
        state.smsRequestAt = 0;
        if (state.customerPhone) state.confirmedPhone = state.customerPhone;
      }
      if (delivery === 'email' || delivery === 'e mail' || delivery === 'both') {
  ...
  if (type === 'conversation.item.input_audio_transcription.completed') {
    ...
    if (state.pendingContactConfirmation) {
      ...
      if (isAffirmativeBookingConfirmation(transcript)) {
        ...
        pushInternalCallContext(state, `The caller explicitly confirmed the ${pending.type} ${pending.value}. Continue the pending estimate/booking flow using that exact value without asking again.`);
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.7' }));
    return;
  }
