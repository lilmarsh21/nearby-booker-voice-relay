...
async function executeNbbTool(state, name, callId, args = {}) {
  ...
  if (['nbb_prepare_booking', 'nbb_send_estimate', 'nbb_request_callback'].includes(name) && toolArgs?.customer && typeof toolArgs.customer === 'object') {
    ...
    const usesEmail = name === 'nbb_prepare_booking' || (name === 'nbb_send_estimate' && ['email', 'both'].includes(normalizedText(toolArgs.delivery || '')));
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.8' }));
    return;
  }
