...
async function refreshTranscriptPermission(state) {
  ...
  try {
    ...
    // Privacy-first fallback: if the live tenant permission cannot be verified,
    // do not transport transcript content. The synced snapshot is retained only
    // for diagnostics and the call itself continues normally.
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.18' }));
    return;
  }
