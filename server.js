...
function asksForTextMessage(text) {
  ...
  const phrases = [
    ...
    'send me that in a text', 'sms me', 'send me an sms', 'text that to me',
...
function asksForEmailMessage(text) {
  ...
  const phrases = [
    ...
    'send that to my email', 'email that to me', 'can you email', 'could you email'
  ];
  return phrases.some((phrase) => t.includes(phrase));
}
...
function dynamicVoiceResponseRules(state) {
  ...
  if (state.rules?.quotes_enabled === false) {
    rules.push('PHONE PRICING IS OFF: do not state or infer service prices, line-item costs, quote totals, appointment totals, minimum charges, or grand totals. A savings_amount/spoken_savings explicitly returned for a savings date is allowed and should be stated because it reveals only the amount saved.');
  }
  return rules.join(' ');
}
...
function afterToolResponseInstructions(state, name, result) {
  const parts = [
    'Continue naturally now using only the exact Nearby Booker tool result. Do not wait for the caller to prompt you. If the tool validated or booked something, state only what the result confirms.'
  ];
...
async function beginOpenAI(state, sessionToken) {
  ...
  openaiWs.on('open', () => {
    ...
    // The customer-facing greeting is supplied separately from internal configuration.
    requestModelResponse(
      state,
...
async function executeNbbTool(state, name, callId, args = {}) {
  ...
  if (name === 'nbb_get_quote' && state.rules?.quotes_enabled === false) {
    acknowledgeFunction(state, callId, {
      success: false,
      code: 'voice_pricing_disabled',
      message: 'Phone Agent pricing is disabled for this tenant.',
    });
    requestModelResponse(
      state,
      'Do not provide or estimate a price. Briefly explain that pricing is not being provided over the phone and continue helping with services or scheduling.',
      'pricing_disabled',
      true
    );
    return;
  ...
  if (name === 'nbb_prepare_booking' && toolArgs?.customer && typeof toolArgs.customer === 'object') {
    state.customerName = String(toolArgs.customer.name || '');
    state.customerPhone = String(toolArgs.customer.phone || '');
    state.customerEmail = String(toolArgs.customer.email || '');
  }
  ...
  try {
    ...
    const failureInstructions = name === 'nbb_get_quote' && state.rules?.quotes_enabled === false
      ? 'Pricing is disabled for this Phone Agent. Do not provide or estimate a price. Briefly say pricing is not being provided over the phone and continue helping with services or scheduling.'
...
function handleOpenAIEvent(state, raw) {
  ...
  if (type === 'conversation.item.input_audio_transcription.completed') {
    ...
    if (transcript) {
      if (asksForTextMessage(transcript)) state.smsRequestAt = Date.now();
      if (asksForEmailMessage(transcript)) state.emailRequestAt = Date.now();
      reportVoiceEvent(state, {
        kind: 'message',
        role: 'user',
        content: transcript,
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.2' }));
    return;
  }
