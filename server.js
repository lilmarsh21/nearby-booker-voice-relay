...
async function refreshTranscriptPermission(state) {
  ...
  try {
    ...
    // Backward-compatible fallback during staggered deploys. Once NBB/Proxy are
    // updated, the tenant-local setting becomes authoritative on every call.
    state.rules.transcripts_enabled = syncedValue;
    log('warn', 'Voice transcript permission refresh unavailable; using synced setting', {
      callSid: state.callSid,
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.17' }));
    return;
  }
