...
function asksForTextMessage(text) {
  const t = normalizedText(text);
  if (!t) return false;
  if (['text', 'sms', 'text message'].includes(t)) return true;
  const phrases = [
...
function asksForEmailMessage(text) {
  const t = normalizedText(text);
  if (!t) return false;
  if (['email', 'e mail'].includes(t)) return true;
  const phrases = [
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.3' }));
    return;
  }
