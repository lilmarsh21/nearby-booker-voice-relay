...
function stopAutomaticBargeCandidate(state) {
  ...
  clearAutomaticBargeTimer(state);
  if (hadPendingTimer && !state.autoBargeTriggered) {
    state.autoFalseStarts = Math.min(5, state.autoFalseStarts + 1);
    reportVoiceEvent(state, {
      kind: 'diagnostic',
...
function handleOpenAIEvent(state, raw) {
  ...
  if (type === 'input_audio_buffer.speech_started') {
    ...
    // Manual modes retain the previous immediate barge-in behavior.
    clearContinuationWatchdog(state);
    state.continuationRetryCount = 0;
  ...
  if (type === 'input_audio_buffer.speech_stopped') {
    if (automaticInterruptionEnabled(state)) stopAutomaticBargeCandidate(state);
    return;
  }
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.13' }));
    return;
  }
