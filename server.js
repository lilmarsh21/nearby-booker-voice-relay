...
function isNoMoreServicesReply(text) {
  ...
  const exact = new Set(['no', 'nope', 'nah', "that's all", 'thats all', "that's it", 'thats it', 'nothing else', 'just that', 'only that']);
  if (exact.has(t)) return true;

  // Never close collection when the caller is clearly rejecting/changing one
  // service while requesting another. These guards run before the broader
  // natural-language closings below.
  if (/\b(?:but|however|except|besides)\b.*\b(?:add|also|need|want|include|clean|service|carpet|upholstery|vent|couch|sofa|sectional|stair|tile)\b/.test(t)) return false;
  if (/\b(?:add|include)\b.*\b(?:service|cleaning|cleaned|treatment|vent|carpet|upholstery|couch|sofa|sectional|stair|tile)\b/.test(t)) return false;
  if (/\b(?:also need|also want|need another|want another|another service)\b/.test(t)) return false;
  if (/\bno (?:not|instead of) (?:that|this)\b/.test(t)) return false;

  // Natural polite/frustrated negatives should close service collection when
  // the reply itself ends at "no/no thanks". Examples from live calls include
  // "for the second time, no" and "for the third time now, no thanks".
  if (/^(?:(?:for the (?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|\d+(?:st|nd|rd|th)) time)(?: now)? )?(?:again )?no(?: thanks| thank you)?$/.test(t)) return true;
  if (/^(?:i already said |i said |i told you )no(?: thanks| thank you)?$/.test(t)) return true;

  // "No" followed by an explicit move-on instruction is still a definitive
  // answer to the additional-services question. Keep the accepted continuations
  // intentionally narrow so a new service request cannot be swallowed.
  if (/^no(?: thanks| thank you)? (?:(?:let's|lets) )?(?:go ahead(?: and)? )?(?:move on|continue|proceed)$/.test(t)) return true;
  if (/^no(?: thanks| thank you)? (?:(?:let's|lets) )?go ahead$/.test(t)) return true;
  if (/^no(?: thanks| thank you)? (?:(?:let's|lets) )?(?:go ahead(?: and)? )?(?:schedule|book)(?: this| it| the appointment| the job| what we have| what we've got| what weve got)?$/.test(t)) return true;
  if (/^no(?: thanks| thank you)? just (?:schedule|book)(?: this| it| the appointment| the job| what we have| what we've got| what weve got)$/.test(t)) return true;

  // Do not let a negated "that's all" sentence accidentally close collection.
  if (/\b(?:don't|dont|do not) think (?:that's|thats|that is) all\b/.test(t)) return false;

  // Accept natural closings with harmless courtesy words at the end, while
  // avoiding replies that continue into another requested service.
  if (/(?:^| )(?:i think )?(?:that's|thats|that is) (?:all|it|everything)(?: (?:thanks|thank you))?$/.test(t)) return true;
  if (/^no (?:that's|thats|that is) (?:all|it|everything)(?: (?:thanks|thank you))?$/.test(t)) return true;

  const phrases = [
    'no other service', 'no other services', 'no more service', 'no more services',
    "don't need anything else", 'dont need anything else', 'do not need anything else', 'nothing else today',
    'that will be all', 'that would be all', 'just those', 'only those'
  ];
  return phrases.some((phrase) => t.includes(phrase));
}
...
function dynamicVoiceResponseRules(state) {
  ...
  if (!state.serviceCollectionClosed) {
    rules.push('MULTI-SERVICE GATE IS OPEN: after capturing a service and its required quantity/variant, ask exactly once whether the caller wants to add any other services BEFORE moving to service address, quote delivery, availability, or booking. Do not assume one service means the order is complete.');
  } else {
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.24' }));
    return;
  }
