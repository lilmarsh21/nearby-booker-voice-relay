...
async function executeNbbTool(state, name, callId, args = {}) {
  ...
  if (name === 'nbb_commit_booking') {
    ...
    if (!confirmationFresh) {
      ...
      safeSend(state.openaiWs, { type: 'response.create', response: { instructions: 'Read back the validated booking review and ask the caller for a clear yes before trying to submit it.' } });
      return;
    }
  ...
  try {
    ...
    safeSend(state.openaiWs, {
      type: 'response.create',
      response: { instructions: 'Continue naturally using only the exact Nearby Booker tool result. If the tool validated or booked something, state only what the result confirms.' },
    });
  } catch (error) {
    ...
    safeSend(state.openaiWs, {
      type: 'response.create',
      response: { instructions: 'The Nearby Booker tool failed. Briefly explain that the requested check or action could not be completed, do not invent a result, and continue by asking for any missing or corrected information.' },
    });
  }
}
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.0' }));
    return;
  }
