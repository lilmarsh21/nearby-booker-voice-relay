...
function armSilenceHangupFromLastActivity(state) {
  ...
  if (state.callerSpeechActive || state.aiAudioActive) return;
...
async function executeSilenceHangup(state) {
  if (state.ended || state.silenceHangupInProgress || state.callerSpeechActive || state.aiAudioActive) {
    armSilenceHangupFromLastActivity(state);
    return;
  }
  const seconds = silenceHangupSeconds(state);
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.14' }));
    return;
  }
