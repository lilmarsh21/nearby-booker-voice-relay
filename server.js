...
function adaptiveBargeGuardMs(state) {
  return Math.max(180, Math.min(460, 200 + (state.autoFalseStarts * 55)));
}
...
function automaticSpeechEnergyLooksReal(state) {
  ...
  const ratio = Math.min(1.85, 1.30 + (state.autoFalseStarts * 0.10));
  const required = Math.max(260, floor * ratio);
  const above = frames.filter((x) => x.rms >= required).length;
  const peak = Math.max(...frames.map((x) => x.rms));
  return peak >= required && above >= Math.min(3, frames.length);
}
...
function isNoMoreServicesReply(text) {
  ...
  const phrases = [
    ...
    "don't need anything else", 'do not need anything else', 'nothing else today',
...
async function refreshTranscriptPermission(state) {
  const syncedValue = Boolean(state.rules?.transcripts_enabled);
  ...
  try {
    ...
    if (enabled !== syncedValue) {
      log('info', 'Voice transcript permission refreshed from tenant', {
        callSid: state.callSid,
        enabled,
      });
    }
    ...
    state.rules.transcripts_enabled = syncedValue;

    // Backward compatibility for tenants that predate the live permission probe.
    // An explicit nbb_voice_tool_unknown response proves the tenant does not know
    // nbb_get_transcript_permission; it is not a transport/auth failure. In that
    // one case, trust the already-signed/synced transcript flag from bootstrap
    // and continue using the long-standing nbb_log_voice_event path. Tenant NBB
    // still makes the final storage decision for every event.
    if (String(error?.code || '') === 'nbb_voice_tool_unknown') {
      state.transcriptPermissionVerified = true;
      log('info', 'Voice transcript permission probe unsupported by tenant; using synced transcript setting', {
        callSid: state.callSid,
        enabled: syncedValue,
        code: error.code,
      });
    ...
    // Privacy-first behavior for genuine verification failures: network, auth,
    // timeout, malformed response, or any other unexpected error must not cause
    // transcript content to be transported solely from a stale snapshot.
    log('warn', 'Voice transcript permission refresh unavailable; transcript transport remains unverified', {
      callSid: state.callSid,
      code: error?.code || 'transcript_permission_refresh_failed',
    });
    return false;
  }
}
...
function reportVoiceEvent(state, payload = {}) {
  if (!state.transcriptPermissionVerified || !state.rules?.transcripts_enabled || !state.sessionId || state.ended) return Promise.resolve();
...
async function beginOpenAI(state, sessionToken) {
  ...
  const transcriptEnabled = await refreshTranscriptPermission(state);
  if (transcriptEnabled) {
    // Create/verify the tenant-local Phone transcript row immediately at call
    // bootstrap. If persistence fails, the Relay now logs the exact tool code
    // instead of silently treating an unconfirmed write as success.
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.19' }));
    return;
  }
