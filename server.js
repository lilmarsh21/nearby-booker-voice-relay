...
function isNoMoreServicesReply(text) {
  ...
  // Natural polite/frustrated negatives should close service collection, but only
  // when "no thanks/no thank you" is the end of the reply. This avoids treating
  // "no thanks, but I also need upholstery" as a completed service list.
  if (/(?:^| )no (?:thanks|thank you)$/.test(t)) return true;
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({ ok: true, service: 'nearby-booker-voice-relay', version: '0.2.22' }));
    return;
  }
